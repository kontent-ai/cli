...
export const searchDocs = async (
  ...
  takeBest(await fetchDocs(deps, params, search), "page", params);

export const getEndpointDetails = async (
  params: DocsParams,
  deps: DocsDeps,
): Promise<Result<ReadonlyArray<EndpointDetails>, DocsError>> =>
  takeBest(await fetchDocs(deps, params, endpointDetails), "endpoint", params);

export const getObjectDetails = async (
  params: DocsParams,
  deps: DocsDeps,
): Promise<Result<ReadonlyArray<ObjectDetails>, DocsError>> =>
  takeBest(await fetchDocs(deps, params, objectDetails), "object", params);
...
const takeBest = <T>(
  ...
  flatMap(result, (candidates) => {
    if (candidates.length === 0) {
      return err({
        kind: "no-match",
        message: `No ${subject} matches "${params.query}". Try different wording, or run kontent docs search "${params.query}" to see what the docs cover.`,
      });
    }
