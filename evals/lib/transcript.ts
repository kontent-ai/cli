...
export const buildTaskTrace = (run: AgentRun): TaskTrace => {
  ...
  return {
    ...
    helpLookupCount: toolCalls.filter((call) => call.isHelpLookup).length,
    docsLookupCount: toolCalls.filter((call) => call.isDocsLookup).length,
    webFetchCount: toolCalls.filter((call) => call.tool === "WebFetch").length,
    failedCallCount: toolCalls.filter((call) => call.outcome === "failed").length,
