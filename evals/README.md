...
checks in `evals/tasks/<id>/task.ts` grade the resulting environment state afterwards. Runs through
Vitest, but not as part of `pnpm test` or CI - it costs money and needs a cloned environment, so it
...
- `report.md` - the same summary as a table, plus a friction section (most failed commands, most
  help lookups, every web fetch) and every task's final agent reply. Nice-to-have:
